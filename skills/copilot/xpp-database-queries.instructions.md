...
## SysDa Framework â€” fluent query API

SysDa is the modern X++ query API â€” fluent and object-oriented. Use it when query shape depends on runtime conditions or when building reusable framework logic.

**Core classes:**
- `SysDaQueryObject` â€” root query builder; set table buffer via constructor.
- `SysDaSearchStatement` â€” execute + iterate; `SysDaFindStatement` â€” `firstOnly` equivalent.
- `SysDaUpdateStatement` / `SysDaInsertStatement` / `SysDaDeleteStatement` â€” set-based ops.
...
**Joins:** `qe.joinClause(SysDaJoinKind::InnerJoin, joinQe)` â€” supports `InnerJoin`, `OuterJoin`, `ExistsJoin`, `NotExistsJoin`.

**SysDa vs `select` â€” decision:**

| Situation | Preferred |
|---|---|
| Static, compile-time query | `select` / `while select` â€” cleaner, compile-time field validation |
...
## Query Object Model â€” `Query` / `QueryRun`
