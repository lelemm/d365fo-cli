...
| `FormDataSource.active()` | Cursor moves to a new record â€” update dependent UI |
...
- `FormDataSource.research(retainPosition: true)` â€” refresh grid, keep cursor position.
- `element.args()` â€” access caller context (menu item, record, enum parameter).
- `FormDataSource.queryBuildDataSource()` â€” underlying `QueryBuildDataSource` for runtime range manipulation.
...
- Cannot add new methods via CoC on `formdatasourcestr`/`formdatafieldstr`/`formControlStr` â€” only wrap methods that already exist.
...
# Output menu item â€” triggers a report
...
**Hard rules:**
- One menu item per AOT type (`AxMenuItemDisplay`, `AxMenuItemAction`, `AxMenuItemOutput`) â€” naming convention `<ObjectName>MenuItem` or `<ObjectName>Action`.
- Do not create an `Action` menu item pointing to a form â€” use `Display`.
