...
## Metadata BP lessons from VS builds

Some BP failures come from metadata shape, not X++ source:

- New `AxMenuItemAction` and `AxMenuItemDisplay` metadata should include
  `<DisabledResource>0</DisabledResource>` and
  `<NormalResource>0</NormalResource>` unless a real supported image resource
  is selected. Missing resource nodes can raise `BPErrorMissingOrUnsupportedImage`.
- Standalone enums should label every `AxEnumValue`; unlabeled enum values can
  be hard to inspect in Visual Studio and may create downstream UI/BP noise.
- Tables should use concrete field subtypes such as `AxTableFieldString`,
  `AxTableFieldDate`, `AxTableFieldReal`, `AxTableFieldInt64`, and
  `AxTableFieldEnum`; bare `AxTableField` nodes can deserialize as abstract
  metadata.
- After adding or renaming labels, run
  `d365fo analyze completeness <ProjectOrModelPath> --resolve-labels --output table`
  before relying on BP results.
