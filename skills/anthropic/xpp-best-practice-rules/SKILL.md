...
When the user asks a **functional** question â€” "which classes print a free invoice", "which classes process sales orders" â€” translate it into **2â€“4 English keyword fragments** and run a single targeted batch search. Always add `--kind class` (or the relevant kind) to avoid scanning all 13 object types.

```sh
# âœ… CORRECT â€” kind-filtered, fast
d365fo search batch FreeTextInvoice PrintFreeTxt FreeInv --kind class --output json

# âŒ WRONG â€” full scan across Tables/Classes/EDTs/Enums/Forms/â€¦ slow and noisy
...
- Use the **English AOT name** (`FreeTextInvoice`, not `volnÃ¡ faktura`).
