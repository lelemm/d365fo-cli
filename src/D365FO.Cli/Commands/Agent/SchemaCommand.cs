...
public sealed class SchemaCommand : Command<SchemaCommand.Settings>
{
    ...
    [
        ...
        C("prepare change", "Single-round change context + grounding token.", ["<OBJECT>"], ["--method", "--goal", "--output"], ["prepare_change"], true),
        C("prepare create", "Single-round new-object context + grounding token.", ["<NAME>"], ["--type", "--field", "--goal", "--output"], ["prepare_create"], true),
        C("validate xpp", "Offline X++/XML best-practice validator.", ["[FILE]"], ["--code-type", "--context", "--output"], ["validate_xpp"], true),
        C("validate references", "Verify every identifier in X++ code against the index.", ["[FILE]"], ["--output"], ["resolve_references"], true),
        C("validate form-pattern", "Structural form-pattern validator (FP001-FP010) over AxForm XML.", ["[FILE]"], ["--output"], ["validate_form_pattern"], true),
        C("get form-pattern", "Form pattern spec catalog (structure tree, versions, when-to-use).", ["[NAME]"], ["--output"], ["get_form_pattern_spec", "get_form_patterns"], true),
